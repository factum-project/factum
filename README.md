...
- **Corroboration detection**: When a second agent asserts the same fact, `factum_assert` returns `corroborated` (not an error) — independent agreement is the most valuable multi-agent signal
- **Conflict arbitration**: LatestWins / HighestAuthority / Unanimous / WeightedVote — pattern-aware grouping ensures value conflicts trigger arbitration; returns `Ambiguous` when it cannot uniquely resolve
