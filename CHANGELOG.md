...
### Added
- **Cascade retraction depth limit**: `retract_with_depth()` method with configurable `max_cascade_depth` parameter (default: 100). Returns `RetractResult` with `truncated` flag and `depth_reached` counter. Prevents cascade explosion in large knowledge bases. MCP `factum_retract` tool now accepts optional `max_cascade_depth` parameter. 5 new tests: truncation, no-truncation, depth-zero, backward compatibility, cycle safety.
- **RetractResult struct**: New return type for retract operations, including retracted node IDs, truncated flag, and depth reached. `retract()` remains backward-compatible (wraps `retract_with_depth` with default depth).
- **StoreError::CascadeLimitExceeded**: New error variant for cascade depth limit exceeded (mapped to JSON-RPC internal error).
- **Multi-agent demo script** (`docs/demo/multi-agent-demo.md`): Step-by-step demo showing 3 agents writing conflicting facts, WeightedVote resolution, Ambiguous refusal, and cascade retraction with depth limit. No new code needed — uses existing MCP tools.
...
- **Large-scale deployment guidance**: New "Scaling to 10+ Agents" section in `docs/multi-agent-usage.md` covering trust tiers, cascade depth protection, garbage metadata defense (min_conf + permission isolation + WeightedVote), and write coordination patterns.
